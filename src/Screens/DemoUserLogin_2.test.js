...
describe("DemoUserLogin_2 Component", () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(languageActions, "fetchGetLanguageMapping")
      .mockImplementation(() => ({ type: "MOCK_LANG" }));
    jest
      .spyOn(languageActions, "fetchGetLevelNameLanguageMapping")
      .mockImplementation(() => ({ type: "MOCK_LEVEL_LANG" }));
  ...
  it("renders loader and triggers login with query params", async () => {
    const { getByRole } = renderWithRouter(
      "/test?demoUserId=demo123&age=10&gender=male&language=english"
    );
  ...
  it("handles Tamil language change via query", async () => {
    renderWithRouter(
      "/test?demoUserId=demo456&age=8&gender=female&language=tamil&userType=guest"
    );
    ...
    await waitFor(() => {
      ...
      expect(
        languageActions.fetchGetLevelNameLanguageMapping
  ...
  it("defaults to English for unrecognized language", async () => {
    renderWithRouter(
      "/test?demoUserId=demo789&age=9&gender=male&language=french"
    );
  ...
  it("handles failed login response", async () => {
    ...
    renderWithRouter(
      "/test?demoUserId=demo000&age=7&gender=female&language=english"
    );
  ...
  it("handles missing user ID in doConnect response", async () => {
    ...
    renderWithRouter(
      "/test?demoUserId=demo101&age=6&gender=female&language=english"
    );
  ...
  it("sets userAge and userGender from query if landingFrom is set", async () => {
    ...
    renderWithRouter(
      "/test?demoUserId=demo777&age=14&gender=male&language=sinhala"
    );
  ...
  it("updates deviceWidth on window resize", async () => {
    const { container } = renderWithRouter(
      "/test?demoUserId=resizeTest&age=10&gender=male&language=english"
    );
  ...
  it("calls getLangugeBaseData with correct ID for Tamil", async () => {
    ...
    renderWithRouter(
      "/test?demoUserId=demoX&age=10&gender=female&language=tamil"
    );
    ...
    await waitFor(() => {
      expect(spy).toHaveBeenCalledWith(
        expect.objectContaining({
          label: "English",
          value: "dbc995a7-0715-4c80-aeef-35f77e9fb517",
        })
      );
    });
  ...
  it("handles malformed doConnect response object", async () => {
    ...
    renderWithRouter(
      "/test?demoUserId=demo404&age=8&gender=male&language=english"
    );
  ...
  it('follows "demo" logic path when landingFrom is "demo"', async () => {
    ...
    renderWithRouter(
      "/test?demoUserId=demoPath&age=15&gender=male&language=sinhala"
    );
  ...
  it("parses location.search and extracts correct values", async () => {
    ...
    renderWithRouter(
      "/test?demoUserId=test123&age=12&gender=female&language=english&userType=guest"
    );
